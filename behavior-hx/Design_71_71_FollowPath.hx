...
class Design_71_71_FollowPath extends ActorScript
{
	...
	override public function init()
	{
		...
		if((_JumptoStart && !((_Coordinates.length == 0))))
		{
			...
			if((_XY.length >= 2))
			{
				actor.setX((_XY[Std.int(0)] - (actor.getWidth()/2)));
				actor.setY((_XY[Std.int(1)] - (actor.getHeight()/2)));
			}
		}
			...
			if(wrapper.enabled)
			{
				...
				else
				{
					...
					if((_CurrentNode < _Coordinates.length))
					{
						...
						if((_XY.length >= 2))
						{
							_XVar = asNumber(_XY[Std.int(0)]);
							propertyChanged("_XVar", _XVar);
							_YVar = asNumber(_XY[Std.int(1)]);
							propertyChanged("_YVar", _YVar);
							if((_XY.length >= 3))
							{
								_CurrentSpeed = asNumber(_XY[Std.int(2)]);
							...
							_DistanceX = asNumber((_XVar - actor.getXCenter()));
							propertyChanged("_DistanceX", _DistanceX);
							_DistanceY = asNumber((_YVar - actor.getYCenter()));
				...
				if((sceneHasBehavior("Game Debugger") && asBoolean(getValueForScene("Game Debugger", "_Enabled"))))
				{
					...
					for(item in cast(_Coordinates, Array<Dynamic>))
					{
						...
						if(_First)
						{
							_First = false;
							propertyChanged("_First", _First);
							_FirstX = asNumber(_XY[Std.int(0)]);
							propertyChanged("_FirstX", _FirstX);
							_FirstY = asNumber(_XY[Std.int(1)]);
						...
						else
						{
							...
							_PreviousX = asNumber(_XVar);
							propertyChanged("_PreviousX", _PreviousX);
							_PreviousY = asNumber(_YVar);
							propertyChanged("_PreviousY", _PreviousY);
						}
