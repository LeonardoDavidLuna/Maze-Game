...
class Design_75_75_ReloadSceneOnDeath extends ActorScript
{
	...
	public function _customEvent_Reload():Void
	{
		if(!(isTransitioning()))
		{
			...
			{
				reloadCurrentScene(createFadeOut(_OutTime, Utils.getColorRGB(0,0,0)), createFadeIn(_InTime, Utils.getColorRGB(0,0,0)));
			}
			else if((_TransitionStyle == "Blinds"))
			{
				reloadCurrentScene(createBlindsOut(_OutTime, Utils.getColorRGB(0,0,0)), createBlindsIn(_InTime, Utils.getColorRGB(0,0,0)));
			}
			else if((_TransitionStyle == "Bubbles"))
			{
				reloadCurrentScene(createBubblesOut(_OutTime, Utils.getColorRGB(0,0,0)), createBubblesIn(_InTime, Utils.getColorRGB(0,0,0)));
			}
			else if((_TransitionStyle == "Spotlight"))
			{
				reloadCurrentScene(createCircleOut(_OutTime, Utils.getColorRGB(0,0,0)), createCircleIn(_InTime, Utils.getColorRGB(0,0,0)));
			}
			else if((_TransitionStyle == "Blur"))
			{
				reloadCurrentScene(createPixelizeOut(_OutTime, Utils.getColorRGB(0,0,0)), createPixelizeIn(_InTime, Utils.getColorRGB(0,0,0)));
			}
			else if((_TransitionStyle == "Box"))
			{
				reloadCurrentScene(createRectangleOut(_OutTime, Utils.getColorRGB(0,0,0)), createRectangleIn(_InTime, Utils.getColorRGB(0,0,0)));
			}
